...
SPATIAL_VAR_FRACTION          = 0.3
TREND_VAR_FRACTION            = 0.7
SPATIAL_VAR_FALLBACK          = 300.0
TREND_VAR_FALLBACK            = 700.0
...
#   temporal_ls = 7d, trend_ls = 30d gives a 4x separation so the Kalman
#   filter can split slow trend from faster x-shape evolution.
...
# --- Model size --------------------------------------------------------------
# N=30: covers (x_hi - x_lo) ~ 0.6 with spacing ~0.02 so monthly windows of
# width ~0.1 see ~5 effective inducing points.  No reproject needed.
N_INDUCING = 30
...
def compute_km_total_var(km_dir, window_list, phase_a_si,
                         x_range=None,
                         drift_trim_pct=0.02,
                         console=None):
    ...
    if x_range is not None:
        ...
        in_range = (
            (km_all['bin_center'] >= x_lo_km) &
            (km_all['bin_center'] <= x_hi_km)
        )
...
def main() -> None:
    import sys
    console = Console(file=open(sys.stdout.fileno(), mode='w',
    ...
    gp_output_dir = os.path.join(
        GP_OUTPUT_DIR_ROOT, f'{phase_gp_seconds_interval}s_trend', 'no_hp',
    )
    ...
    x_prev, dx, dt, dt_t = load_series(
        snapped_start, snapped_end, phase_gp_seconds_interval,
        kernel_half_width=kernel_half_width,
        trim_quantile=trim_quantile,
        window_type=window_type,
    )
    ...
    sl_init = (
        ...
        else (x_hi - x_lo) / max(_n_inducing_eff, 1)
    )
    ...
    total_var = compute_km_total_var(
        km_dir, window_list, phase_a_seconds_interval,
        x_range=x_range_global, console=console,
    )
    ...
    for w_idx in labels_ts.index:
        ...
        # Per-window x-range for topology evaluation (clipped to global).
        x_w_width = max(float(x_w.max() - x_w.min()), 1e-4)
        margin    = 0.05 * x_w_width
        topo_range = (
            max(x_lo, float(x_w.min()) - margin),
            min(x_hi, float(x_w.max()) + margin),
        )
    ...
    plot_topology_snapshots(
        ...
        n_grid=N_GRID, n_samples=N_SAMPLES, rng=rng,
    )
    ...
    plot_drift_with_km(
        model, snapshots,
        snapped_start, snapped_end,
        phase_a_seconds_interval, phase_gp_seconds_interval,
        OUTPUT_DIR,
        os.path.join(gp_output_dir, f'{stem}_drift_km.png'),
        spatial_var_source='km_split',
        hp_opt_mode='none',
        use_reproject=False,
        n_grid=N_GRID, rng=rng,
    )
